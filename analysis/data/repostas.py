...
class Resposta:
    ...
    def treat_order_fields(self, field):
        ...
        for idx, row in df_aux.iterrows():
            df_aux1 = pd.concat([df_aux1, pd.DataFrame({field: [[val for val in row]]})])
    ...
    def treat_vars(self):
        ...
        self.df_variaveis.rename(columns={'Carimbo de data/hora': 'atualizacao_dia', 'Município': 'municipio'}, inplace=True)
    ...
    def get_capacidade(self, prefix):
        self.treat_vars()
        cols = [col for col in self.df_variaveis if col.startswith(prefix)] + ['municipio']
        self.df_variaveis['update_date'] = pd.to_datetime(self.df_variaveis['atualizacao_dia'], format='%d/%m/%Y %H:%M:%S')
